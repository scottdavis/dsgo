...
func RunRedisAgentExample(ctx context.Context, logger *logging.Logger, dspyConfig *core.DSPYConfig, redisConfig RedisConfig) {
	...
	ctx = context.Background() // Create a new context for TTL demonstration
	expiringCtx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func CreateKnowledgeBaseWorkflow(dspyConfig *core.DSPYConfig, redisStore memory.Memory) (*workflows.ChainWorkflow, error) {
...
func (m *KnowledgeBaseModule) Process(ctx context.Context, inputs map[string]any, opts ...core.Option) (map[string]any, error) {
	...
	memory, ok := ctx.Value(core.MemoryKey).(agents.Memory)
	if !ok {
	...
	case "store_with_ttl":
		// Check if memory supports TTL
		ttlMemory, ok := memory.(memory.Memory)
		if !ok {
			return nil, fmt.Errorf("memory does not support TTL")
		}
		if err := ttlMemory.StoreWithTTL(ctx, key, content, time.Duration(ttl)*time.Second); err != nil {
...
// Main function to demonstrate Redis storage example
func main() {
	...
	// Create LLM
	llmConfig := llms.NewOllamaConfig("http://localhost:11434", "llama3")
