...
// Test for rate limiting information in error metadata field
func TestOpenRouterRateLimitHandlingWithErrorMetadata(t *testing.T) {
	// Create a test server that simulates rate limiting with error metadata
	requestCount := 0
	resetTime := time.Now().Add(2 * time.Second).UnixMilli()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Return a 200 OK but with rate limit info in error metadata
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		
		// Return response with rate limit info in error metadata
		w.Write([]byte(fmt.Sprintf(`{
			"error": {
	...
	// Make a request that should detect rate limiting from error metadata
	_, err = llm.Generate(ctx, "Test request with error metadata rate limiting")
	
	// We expect an error because our code detects rate limiting from error metadata
	require.Error(t, err, "Generate should detect rate limiting from error metadata")
	require.Contains(t, err.Error(), "rate limited (from error metadata)")
	
	// Test that we made exactly one request
	assert.Equal(t, 1, requestCount, "Should have made exactly 1 request")
}

// Test for rate limiting information in error metadata field with functions
func TestOpenRouterGenerateWithFunctionsRateLimitHandlingWithErrorMetadata(t *testing.T) {
	// Create a test server that simulates rate limiting with error metadata
	requestCount := 0
	resetTime := time.Now().Add(2 * time.Second).UnixMilli()
	...
	// Make a request that should detect rate limiting from error metadata
	_, err = llm.GenerateWithFunctions(ctx, "Test request with error metadata rate limiting", functions)
	
	// We expect an error because our code detects rate limiting from error metadata
	require.Error(t, err, "GenerateWithFunctions should detect rate limiting from error metadata")
	require.Contains(t, err.Error(), "rate limited (from error metadata)")
